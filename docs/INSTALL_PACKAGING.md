...
```text
dist/zhongkui-waf-master-2.1.2.tar.gz
dist/zhongkui-waf-master-2.1.2.tar.gz.sha256
dist/zhongkui-waf-node-2.1.2.tar.gz
dist/zhongkui-waf-node-2.1.2.tar.gz.sha256
```

也可以显式指定版本和输出目录：

```bash
./scripts/build_release.sh --version 2.1.2 --output-dir /tmp/zhongkui-release
```
...
```bash
sha256sum -c dist/zhongkui-waf-master-2.1.2.tar.gz.sha256
tar -tzf dist/zhongkui-waf-master-2.1.2.tar.gz | grep -E '(^|/)(ssh|\.zhongkui\.private\.env|\.zhongkui\.release\.env)$|conf/system.json'
```
...
不要在已经运行的服务器上重复执行 `install.sh`。解压对应角色包后，执行：
...
脚本会备份旧项目目录，替换代码并保留本机 `conf/system.json`、全局规则、站点配置、证书配置和本地 `admin` 密码。只有 `nginx -t` 通过才会 reload；验证失败会自动回滚。
