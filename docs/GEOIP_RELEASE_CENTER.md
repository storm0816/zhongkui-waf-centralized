...
4. 选择目标 Node，并设置灰度节点、批次大小和批次间隔。
5. 灰度模式先只下发 1 台；灰度成功后才继续后续批次，失败时自动暂停。
6. 后续批次等待上一批结束并达到间隔后再下发，避免多个 Node 同时下载大文件。
7. Node 下载到临时文件，校验 SHA-256，并使用固定公网 IP 执行真实查询。
8. 校验成功后备份旧 MMDB、原子替换、检查 OpenResty 配置并重载。
9. 失败时自动恢复旧 MMDB；结果通过节点心跳显示在 GeoIP 任务页。
