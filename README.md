...
- 修复同一 Nginx `server` 配置多个域名别名时，集群 CC 域名统计、CC 阈值、CC 命中、攻击日志、封禁通知和敏感数据发现均错误归属到首个 `server_name` 的问题。
- 防护规则仍按 Nginx 选中的站点配置执行；仅运营数据改为使用实际请求 Host，因此不会改变既有规则匹配或拦截行为。

### 2.2.2 发布说明

- Node 程序与 GeoIP 升级器记录失败阶段，并将升级结果沿现有心跳回传到发布中心。
- `nginx -t` 与 Nginx 重载失败会回传经过截断和转义的错误摘要；成功回滚、回滚后校验失败及切换前失败均会明确标识。
- 不新增 Node 服务或提权通道，复用既有 root 权限升级器与发布中心任务详情。
- 新版升级器随程序版本包下发；Node 成功更新至本版后，后续发布失败将显示详细原因。
...
- 系统设置的钉钉通知支持可选“加签密钥”：密钥为空时按普通 Webhook 发送；填写 `SEC` 开头密钥后自动追加钉钉 `timestamp` 与 HMAC-SHA256 签名。管理页仅显示是否已启用加签，密钥不会回传浏览器；可通过“移除加签”明确恢复普通发送。
...
```bash
tar -xzf zhongkui-waf-node-2.2.2.tar.gz
cd zhongkui-waf-node-2.2.2
...
```bash
tar -xzf zhongkui-waf-master-2.2.2.tar.gz
cd zhongkui-waf-master-2.2.2
...
2. 在“程序版本”创建当前版本的不可变快照，例如 `2.2.2`。已存在的版本快照不能覆盖。
...
安装包会生成到 `dist/`：`zhongkui-waf-master-2.2.2.tar.gz` 与 `zhongkui-waf-node-2.2.2.tar.gz`。构建使用本机忽略的 `.zhongkui.release.env` 注入生产 MySQL/Redis、LDAP 与钉钉配置；Node 包不包含 LDAP 配置。Git 模板始终使用 `10.10.10.10` 占位，不包含这些凭据；`scripts/`、Git 和私有环境文件不会进入压缩包。构建后应验证 SHA-256、`install.sh`/`upgrade.sh` 的 `755` 权限、角色 JSON 合法性与 Node 配置未包含 LDAP 字段。完整说明见：[安装包与发布流程](./docs/INSTALL_PACKAGING.md)。
...
- `2.2.2` 的详细失败原因由 Node 升级器生成，需通过发布中心将程序版本下发到 Node。升级器在本次成功切换后更新，详细错误从下一次发布开始生效。
