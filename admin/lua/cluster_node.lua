...
local function listNodes()
    ...
    local master_mmdb = geoip_version.get_local()
        ...
        local sql_data = string.format([[
            SELECT ip,
                   ...
                   COALESCE(NULLIF(rules_version, ''), 'unknown') AS rules_version,
                    ...
                    COALESCE(NULLIF(mmdb_checksum, ''), 'unknown') AS mmdb_checksum,
                    COALESCE(NULLIF(mmdb_status, ''), 'unknown') AS mmdb_status,
                    mmdb_checked_at,
                   ...
                   program_update_at,
                   COALESCE(NULLIF(geoip_update_target, ''), '-') AS geoip_update_target,
                   COALESCE(NULLIF(geoip_update_status, ''), 'idle') AS geoip_update_status,
                   COALESCE(geoip_update_message, '') AS geoip_update_message,
                   geoip_update_at,
                   hostname,
                   last_seen,
    ...
    response.master_mmdb_checksum = master_mmdb.checksum or "unknown"
    response.master_mmdb_status = master_mmdb.status or "unknown"
    response.master_app_version = APP_VERSION
...
local function list_cluster_domains()
    ...
    local rows, err, total = require("sql").list_cc_cluster_domains({
        domain = args.domain,
        status = args.status,
    ...
    return { code = 0, msg = "", count = total or #rows, data = rows }
end

local function save_domain_policy()
    if not user.has_permission("manage") then
        return { code = 403, msg = "permission denied" }
    end
    ngx.req.read_body()
    local values, args_err = ngx.req.get_post_args()
    if not values then
        return { code = 400, msg = args_err or "invalid request" }
    end
    local ok, err = require("sql").save_cc_domain_policy(values.domain, values.threshold, values.duration, values.state)
    if not ok then return { code = 500, msg = err or "save policy failed" } end
    return { code = 0, msg = "保存成功" }
end

-- 近期受攻击节点概览
local function delete_cluster_domain()
    if not user.has_permission("manage") then
        return { code = 403, msg = "permission denied" }
    end
    ngx.req.read_body()
    ...
    local ok, err = require("sql").delete_cc_cluster_domain(values.domain)
    if not ok then return { code = 400, msg = err or "delete domain failed" } end
...
function _M.do_request()
    ...
    elseif uri == "/clusternode/domains" then
        response = list_cluster_domains()
    elseif uri == "/clusternode/domain/policy/save" and ngx.req.get_method() == "POST" then
        response = save_domain_policy()
    elseif uri == "/clusternode/domain/delete" and ngx.req.get_method() == "POST" then
        response = delete_cluster_domain()
