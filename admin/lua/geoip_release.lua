...
local function ensure_release_token(version)
    local key = constants.KEY_REDIS_GEOIP_RELEASE_PREFIX .. version .. ":token"
    local current, get_err = redis_cli.get(key)
    if current and current ~= ngx.null and current ~= "" then return true end
    local saved, save_err = redis_cli.set(key, secure_id("geoip:" .. version), 604800)
...
function _M.do_request()
    ...
    elseif ngx.var.uri == "/geoiprelease/retry" and ngx.req.get_method() == "POST" then
        response = update_deployment("retry")
    elseif ngx.var.uri == "/geoiprelease/skip" and ngx.req.get_method() == "POST" then
