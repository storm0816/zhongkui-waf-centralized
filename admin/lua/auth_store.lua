...
function _M.ensure_schema()
    ...
    -- Existing installations predate the per-user MFA policy column.
    local mfa_columns = mysql.query([[SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='waf_admin_user'
        AND COLUMN_NAME='mfa_required' LIMIT 1]])
    if mfa_columns and not mfa_columns[1] then
        mysql.query("ALTER TABLE waf_admin_user ADD COLUMN mfa_required TINYINT(1) NOT NULL DEFAULT 0 AFTER status")
    end
...
function _M.set_user_mfa_required(user_id, required)
    user_id = tonumber(user_id)
    if not user_id then return nil, "invalid user" end
    if required ~= "on" and required ~= "off" then return nil, "invalid mfa policy" end
    local user = _M.get_user_by_id(user_id)
    if not user then return nil, "user not found" end
    return mysql.query("UPDATE waf_admin_user SET mfa_required=" .. (required == "on" and "1" or "0")
        .. " WHERE id=" .. user_id)
end
...
function _M.list_users()
    _M.ensure_schema()
    return mysql.query([[SELECT u.id,u.username,u.display_name,u.auth_source,u.status,u.mfa_required,u.mfa_enabled,u.last_login_at,
