...
local attack_log_cfg = get_system_config('attackLog') or {}
local RESPONSE_BODY_MAX_BYTES = tonumber(attack_log_cfg.response_body_max_bytes) or 1024

local function truncate_response_body(body)
    if not body then
        return body
    end
    if RESPONSE_BODY_MAX_BYTES <= 0 then
        return ""
    end
    local size = #body
    if size <= RESPONSE_BODY_MAX_BYTES then
        return body
    end
