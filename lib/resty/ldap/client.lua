-- The client only needs lua_pack's unsigned-byte (C) decoder. Keeping this
-- tiny implementation avoids a native runtime dependency in offline installs.
local function bunpack(value, format, position)
    if format ~= "C" then
        return nil, "unsupported unpack format"
    end
    position = position or 1
    local byte = string.byte(value, position)
    if not byte then
        return nil, "input is shorter than requested unpack position"
    end
    return position + 1, byte
end
