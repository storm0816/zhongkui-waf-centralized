...
local filter_compiler
local FILTER = {
    OP_TYPE_AND = "and",
    OP_TYPE_OR = "or",
    OP_TYPE_NOT = "not",
    ITEM_TYPE_SUBSTRING = "substring",
    ITEM_TYPE_SIMPLE = "simple",
    ITEM_TYPE_PRESENT = "present",
    FILTER_TYPE_EQUAL = "equal",
    FILTER_TYPE_APPROX = "approx",
    FILTER_TYPE_GREATER = "greater",
    FILTER_TYPE_LESS = "less",
}
...
local function build_asn1_filter(filter)
    ...
    elseif item_type == FILTER.ITEM_TYPE_SUBSTRING then
        local body = ""
...
local function build_asn1_filters(filter_tbl)
    ...
    if filter_tbl.op_type and filter_tbl.op_type == FILTER.OP_TYPE_NOT and
    ...
    if filter_tbl.op_type and filter_tbl.items and #filter_tbl.items > 1 then
        ...
        return asn1_put_object(
                    filter_tbl.op_type == FILTER.OP_TYPE_AND and 0 or 1, -- 'and' 0 or 'or' 1
                    asn1.CLASS.CONTEXT_SPECIFIC, 1,
                    sub_filter
               )
    end
...
function _M.search_request(base_obj, scope, deref_aliases, size_limit, time_limit,
    ...
    if type(filter) == "table" then
        ...
        -- String filters retain full RFC 4515 support when LPeg is available.
        if not filter_compiler then
            filter_compiler = require("resty.ldap.filter")
        end
        filter_tbl, err = filter_compiler.compile(filter)
