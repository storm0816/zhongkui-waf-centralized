...
local SQL_CREATE_TABLE_WAF_CLUSTER_NODE = [[
    CREATE TABLE waf_cluster_node (
        ...
        app_version VARCHAR(32) COMMENT '程序版本',
        ...
        mmdb_checksum CHAR(64) COMMENT 'GeoIP MMDB SHA256',
        mmdb_status VARCHAR(32) COMMENT 'GeoIP MMDB校验状态',
        mmdb_checked_at DATETIME NULL COMMENT 'GeoIP MMDB校验时间',
        domains_json TEXT NULL COMMENT '节点防护域名',
...
local SQL_CREATE_TABLE_WAF_CLUSTER_DOMAIN = [[
    CREATE TABLE waf_cluster_domain (
        ...
        INDEX idx_cluster_domain_last_seen (last_seen)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
]]

local SQL_CREATE_TABLE_WAF_CLUSTER_NODE_DOMAIN = [[
    CREATE TABLE waf_cluster_node_domain (
        node_ip VARCHAR(64) NOT NULL,
        domain VARCHAR(255) NOT NULL,
        last_seen DATETIME NOT NULL,
        PRIMARY KEY (node_ip, domain),
        INDEX idx_node_domain_domain_seen (domain, last_seen)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
]]

local SQL_CREATE_TABLE_WAF_CC_DOMAIN_POLICY = [[
    CREATE TABLE waf_cc_domain_policy (
        domain VARCHAR(255) NOT NULL PRIMARY KEY,
        threshold_value INT UNSIGNED NOT NULL DEFAULT 1000,
...
local SQL_INSERT_CLUSTER_NODE = [[
    INSERT INTO waf_cluster_node (
        ip, app_version, rules_version, whitelist_version, blacklist_version,
        mmdb_checksum, mmdb_status, mmdb_checked_at, domains_json,
    ...
    ON DUPLICATE KEY UPDATE
        ...
        mmdb_checksum = VALUES(mmdb_checksum),
        mmdb_status = VALUES(mmdb_status),
        mmdb_checked_at = VALUES(mmdb_checked_at),
...
function _M.check_table(premature)
    ...
    local tables = {
        ...
        { name = 'waf_cluster_domain',  sql = SQL_CREATE_TABLE_WAF_CLUSTER_DOMAIN },
        { name = 'waf_cluster_node_domain', sql = SQL_CREATE_TABLE_WAF_CLUSTER_NODE_DOMAIN },
        { name = 'waf_cc_domain_policy', sql = SQL_CREATE_TABLE_WAF_CC_DOMAIN_POLICY },
    ...
    ensure_column("waf_cluster_node", "app_version",
        "ALTER TABLE waf_cluster_node ADD COLUMN app_version VARCHAR(32) NULL COMMENT '程序版本' AFTER ip")
    ...
    ensure_column("waf_cluster_node", "mmdb_checksum",
        "ALTER TABLE waf_cluster_node ADD COLUMN mmdb_checksum CHAR(64) NULL COMMENT 'GeoIP MMDB SHA256' AFTER blacklist_version")
    ensure_column("waf_cluster_node", "mmdb_status",
        "ALTER TABLE waf_cluster_node ADD COLUMN mmdb_status VARCHAR(32) NULL COMMENT 'GeoIP MMDB校验状态' AFTER mmdb_checksum")
    ensure_column("waf_cluster_node", "mmdb_checked_at",
        "ALTER TABLE waf_cluster_node ADD COLUMN mmdb_checked_at DATETIME NULL COMMENT 'GeoIP MMDB校验时间' AFTER mmdb_status")
    ensure_column("waf_cluster_node", "domains_json",
        "ALTER TABLE waf_cluster_node ADD COLUMN domains_json TEXT NULL COMMENT '节点防护域名' AFTER mmdb_checked_at")
...
function _M.report_node_info()
    ...
    local mmdb = geoip_version.get_local()
    local domain_set = {}
    for _, domain in ipairs(config.get_configured_server_names()) do
        domain_set[domain] = true
    end
    for _, domain in ipairs(cc_cluster.get_observed_domains()) do
        domain_set[domain] = true
    end
    local reported_domains = {}
    for domain, _ in pairs(domain_set) do
        reported_domains[#reported_domains + 1] = domain
    end
    table.sort(reported_domains)
    local domains_json = #reported_domains > 0 and cjson.encode(reported_domains) or "[]"
    ...
    local info = {
        ...
        mmdb_checksum = mmdb.checksum,
        mmdb_status = mmdb.status,
        mmdb_checked_at = ngx.localtime(),
...
function _M.write_cluster_nodes_to_mysql()
    ...
    for _, node in ipairs(nodes) do
        ...
        local sql = format(SQL_INSERT_CLUSTER_NODE,
            ...
            quote_sql_str(node.app_version or "unknown"),
            ...
            quote_sql_str(node.mmdb_checksum or "unknown"),
            quote_sql_str(node.mmdb_status or "unknown"),
            quote_sql_str(node.mmdb_checked_at or node.last_seen),
            quote_sql_str(node.domains_json or "[]"),
        ...
        local domains = cjson.decode(node.domains_json or "[]") or {}
        for _, domain in ipairs(domains) do
            domain = tostring(domain):lower()
            if domain ~= "" and #domain <= 255 then
                local domain_res, domain_err = mysql.query("INSERT INTO waf_cluster_domain (domain,first_seen,last_seen,status) VALUES ("
                    .. quote_sql_str(domain) .. ",NOW(),NOW(),'active') ON DUPLICATE KEY UPDATE last_seen=NOW(),status='active'")
                if not domain_res then
                    ngx.log(ngx.ERR, "failed to update cluster domain ", domain, ": ", domain_err)
                end
                local relation_res, relation_err = mysql.query("INSERT INTO waf_cluster_node_domain (node_ip,domain,last_seen) VALUES ("
                    .. quote_sql_str(node.ip) .. "," .. quote_sql_str(domain)
                    .. ",NOW()) ON DUPLICATE KEY UPDATE last_seen=NOW()")
                if not relation_res then
                    ngx.log(ngx.ERR, "failed to update cluster node domain relation: ", relation_err)
                end
            end
        end
        -- Domain-to-node relations are historical ownership records. Keep them
        -- until an administrator explicitly deletes the inactive domain.
    end
...
function _M.publish_cc_domain_policy()
    ...
    local snapshot = { version = ngx.time(), default = { threshold = 1000, duration = 60 }, domains = {} }
    for _, row in ipairs(rows) do
        local item = { threshold = tonumber(row.threshold_value) or 1000, duration = tonumber(row.duration_seconds) or 60 }
        if row.domain == "*" then snapshot.default = item
        elseif row.state == "on" then snapshot.domains[row.domain] = item end
    end
    local encoded = cjson.encode(snapshot)
    if not encoded then return nil, "failed to encode cc domain policy" end
    return redis_cli.set(constants.KEY_REDIS_CC_DOMAIN_POLICY, encoded, -1)
end

function _M.save_cc_domain_policy(domain, threshold, duration, state)
    domain = tostring(domain or ""):lower()
    threshold, duration = tonumber(threshold), tonumber(duration)
    local valid_domain = domain == "*" or domain:match("^[%w][%w%.%-]*[%w]$") or domain:match("^[%w]$")
    if domain == "" or #domain > 255 or not valid_domain
        or not threshold or threshold < 1 or not duration or duration < 1 then
        return nil, "invalid policy"
    end
    state = state == "off" and "off" or "on"
    local sql = format([[INSERT INTO waf_cc_domain_policy (domain,threshold_value,duration_seconds,state)
        VALUES (%s,%d,%d,%s) ON DUPLICATE KEY UPDATE threshold_value=VALUES(threshold_value),
        duration_seconds=VALUES(duration_seconds),state=VALUES(state)]],
        quote_sql_str(domain), threshold, duration, quote_sql_str(state))
    local ok, err = mysql.query(sql)
    if not ok then return nil, err end
    return _M.publish_cc_domain_policy()
end

function _M.delete_cc_cluster_domain(domain)
    domain = tostring(domain or ""):lower()
    if domain == "" or domain == "*" then
        return nil, "invalid domain"
    end

    local rows, err = mysql.query("SELECT status FROM waf_cluster_domain WHERE domain=" .. quote_sql_str(domain) .. " LIMIT 1")
    if not rows then return nil, err end
    if not rows[1] then return nil, "domain not found" end
    if rows[1].status == "active" then
        return nil, "only inactive domains can be deleted"
    end

    local deletes = {
        "DELETE FROM waf_cluster_node_domain WHERE domain=" .. quote_sql_str(domain),
        "DELETE FROM waf_cc_domain_policy WHERE domain=" .. quote_sql_str(domain),
        "DELETE FROM waf_cluster_domain WHERE domain=" .. quote_sql_str(domain)
    }
    for _, sql in ipairs(deletes) do
        local result, delete_err = mysql.query(sql)
...
function _M.list_cc_cluster_domains(filters)
    ...
    local rows, err = mysql.query([[SELECT d.domain,d.status,d.first_seen,d.last_seen,
        ...
        CASE WHEN p.domain IS NULL THEN 'default' ELSE 'custom' END AS policy_source,
        COUNT(DISTINCT nd.node_ip) AS node_count,
        GROUP_CONCAT(DISTINCT nd.node_ip ORDER BY nd.node_ip SEPARATOR ',') AS node_ips
        FROM waf_cluster_domain d
    ...
    for _, row in ipairs(rows) do
        ...
        for offset = 0, 23 do
            local hour = os.date("%Y%m%d%H", now - offset * 3600)
