        ...
        for _, rule_table in pairs(rules) do
            ...
            if match then
                if is_system_option_on("redis") then
                    ...
                    local count, _ = redis_cli.incr(key, duration)
                    ...
                    elseif not count then
                        ...
                        local lcount, _ = limit:incr(key, 1, 0, duration)
                        if not lcount then
                            limit:set(key, 1, duration)
                        elseif lcount >= threshold then
                            if is_rule_exception("cc") then
                                return false
                            end
                            ngx.ctx.is_cc = true
                ...
                elseif line ~= '' then
                    if is_file then
                        ...
                        local from, to = matches(line,
                            [[Content-Disposition:\s*form-data;[\s\S]+filename=["|'][\s\S]+\.(\w+)(?:"|')]], "ijo", nil,
                            1)
