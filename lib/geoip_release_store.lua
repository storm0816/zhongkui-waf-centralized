...
function _M.ensure_tables()
    ...
    for _, column in ipairs(columns) do
        ok, err = ensure_column("waf_geoip_deployment", column[1], column[2])
        if not ok then return nil, err end
    end
    ok, err = ensure_index("waf_geoip_deployment", "idx_geoip_deployment_plan", "(plan_id,batch_no,status)")
    if not ok then return nil, err end
    return true
end
...
function _M.create_deployment(values)
    local sql = string.format([[INSERT INTO waf_geoip_deployment
        (task_id,plan_id,batch_no,is_canary,batch_interval_seconds,release_version,release_checksum,node_ip,status,message,created_by)
        VALUES (%s,%s,%d,%d,%d,%s,%s,%s,'scheduled','等待发布调度',%s)]],
        ...
        quote(values.release_version), quote(values.release_checksum), quote(values.node_ip),
        quote(values.created_by or ""))
    return mysql.query(sql)
end
...
function _M.list_deployments()
    return mysql.query([[SELECT d.task_id,d.plan_id,d.batch_no,d.is_canary,d.batch_interval_seconds,
        d.release_version,d.release_checksum,d.node_ip,d.status,d.message,
        d.created_by,d.created_at,d.updated_at,d.queued_at,d.finished_at,n.hostname,n.mmdb_checksum,
