...
function _M.get_deployment(task_id)
    local rows, err = mysql.query("SELECT task_id,plan_id,release_version,node_ip,status FROM waf_program_deployment"
        .. " WHERE task_id=" .. quote(task_id) .. " LIMIT 1")
    return rows and rows[1] or nil, err
end

function _M.retry_deployment(task_id)
    return mysql.query("UPDATE waf_program_deployment SET status='scheduled',message="
...
function _M.skip_deployment(task_id)
    return mysql.query("UPDATE waf_program_deployment SET status='cancelled',message="
...
function _M.reconcile_deployments()
    local ok, err = mysql.query([[UPDATE waf_program_deployment d JOIN waf_cluster_node n ON n.ip=d.node_ip
        ...
        -- A Node reports its current version, not the outcome of every historical
        -- deployment. Only reconcile tasks that are still actively progressing;
