...
local function write_attack_log_to_redis(log_data)
    ...
    local redis_key = "waf:attack_log:" .. log_data.request_id
...
local function write_ip_block_log_to_redis(log_data)
    local redis_cli = require "redis_cli"
    local redis_key = "waf:ip_block_log:" .. log_data.request_id
    local redis_value, err = cjson_encode(log_data)
    ...
    local ok, err = redis_cli.set(redis_key, redis_value, get_system_config('redis').expire_time)
    if not ok then
        ngx.log(ngx.ERR, "failed to write ip block log to redis: ", err)
    end
end
...
local function write_attack_log()
    ...
    if not is_system_option_on("redis") and is_system_option_on("mysql") then
        local sql_str =
...
local function write_ip_block_log()
    ...
    local log_table = {
        ...
        attack_type = attack_type,
        ip = ip,
        ip_country = country.names[language] or 'unknown',
        ip_province = province.names[language] or 'unknown',
        ip_city = city.names[language] or 'unknown',
        ip_longitude = longitude,
        ip_latitude = latitude,
        start_time = start_time,
        endTime = endTime,
        action = action,
        ip_block_expire_in_seconds = ip_block_expire_in_seconds
    }

    -- 无论 Redis 是否开启，当 ip_block_expire_in_seconds == 0 时，都要将 IP 加入黑名单
    ...
    -- 使用 ngx.timer.at 异步写入 Redis
    ngx.timer.at(0, function()
        write_ip_block_log_to_redis(log_table)
    end)

    -- 当 Redis 未开启且 MySQL 开启时，将 IP 阻断日志写入 MySQL
    if not is_system_option_on("redis") and is_system_option_on("mysql") then
        if ip_block_expire_in_seconds > 0 then
