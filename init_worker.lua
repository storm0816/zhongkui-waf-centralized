...
local function is_cluster_rule_publish_busy(err)
    return err == "cluster rule publish is already in progress"
end
...
local function safe_publish_cluster_rules_snapshot(premature)
    ...
    if not ok and is_cluster_rule_publish_busy(err) then
        ngx.log(ngx.NOTICE, "skip cluster rules publish: another publish is in progress")
        return
    elseif not ok and err and err ~= "not master node" then
...
local function safe_reconcile_master_rule_sources(premature)
    ...
    if not ok and is_cluster_rule_publish_busy(err) then
        ngx.log(ngx.NOTICE, "skip authoritative cluster rules republish: another publish is in progress")
    elseif not ok and err and err ~= "not master node" and err ~= "published release not found" then
        mark_sync(rules_sync_status_dict_key, rules_sync_at_dict_key, "failed")
