...
local mysql_fail_once = true
local captured_sql = {}
package.loaded['mysql_cli'] = {
  query = function(sql)
    captured_sql[#captured_sql + 1] = sql
    if mysql_fail_once then
      mysql_fail_once = false
      return nil, 'mock mysql down'
    end
...
redis_cli.del(constants.KEY_REDIS_RETRY_TRAFFIC_STATS)
redis_cli.del(constants.KEY_REDIS_RETRY_ATTACK_TYPE_DATES)
...
-- Round 1: traffic_stats mysql fail => should enter retry set.
sql.write_traffic_stats_redis_to_mysql()
local retry_traffic = redis_cli.smembers(constants.KEY_REDIS_RETRY_TRAFFIC_STATS) or {}
assert(#retry_traffic == 1, 'retry traffic set should have one member when mysql fails')

-- Replay retry markers => traffic key should be put back into dirty set.
sql.replay_retry_markers()
local dirty_traffic_after_replay = redis_cli.smembers(constants.KEY_REDIS_DIRTY_TRAFFIC_STATS) or {}
assert(#dirty_traffic_after_replay == 1, 'dirty traffic set should be restored by replay')

-- Round 2: mysql recovers => should clear both dirty/retry and delete source key.
sql.write_traffic_stats_redis_to_mysql()
local traffic_deleted = redis_cli.get(traffic_key)
local retry_traffic_after_success = redis_cli.smembers(constants.KEY_REDIS_RETRY_TRAFFIC_STATS) or {}
local dirty_traffic_after_success = redis_cli.smembers(constants.KEY_REDIS_DIRTY_TRAFFIC_STATS) or {}

assert(traffic_deleted == nil, 'traffic key should be deleted after successful sync')
assert(#retry_traffic_after_success == 0, 'retry traffic set should be empty after successful sync')
assert(#dirty_traffic_after_success == 0, 'dirty traffic set should be empty after successful sync')

-- Attack type path should still work in the same run.
...
print('OK: step4 docker verification passed')
