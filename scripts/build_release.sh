...
# Release profiles are often maintained on Windows. Normalize CRLF only in the
# temporary build directory so values do not retain a trailing carriage return.
NORMALIZED_RELEASE_PROFILE="$WORK_DIR/release.env"
tr -d '\r' < "$RELEASE_PROFILE" > "$NORMALIZED_RELEASE_PROFILE"
# The profile is local and ignored by Git. It supplies production package values only.
# shellcheck disable=SC1090
source "$NORMALIZED_RELEASE_PROFILE"
...
build_role_package() {
    ...
    rm -rf "$stage_dir/.git" "$stage_dir/.github" "$stage_dir/.codeartsdoer" "$stage_dir/.codex" \
        "$stage_dir/dist" "$stage_dir/release" "$stage_dir/.release-work" \
